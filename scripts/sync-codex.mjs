...
const keyAliasesSource = join(sourceRoot, "codex-rs/config/src/key_aliases.rs");
...
const [schemaJson, modelsJson, featuresRust, legacyFeaturesRust, keyAliasesRust] = await Promise.all([
  readFile(schemaSource, "utf8").then(JSON.parse),
...
const configKeyAliases = parseConfigKeyAliases(keyAliasesRust);
const tuiKeys = Object.keys(schemaJson.definitions?.Tui?.properties ?? {}).sort();
if (tuiKeys.length === 0) {
  throw new Error(`Could not find the Tui schema properties in ${schemaSource}`);
}
...
function parseConfigKeyAliases(source) {
  ...
  const entries = blocks
    .flatMap((block) => {
      ...
      if (tablePathSource === undefined || !legacyKey || !canonicalKey) {
        return [];
      }
